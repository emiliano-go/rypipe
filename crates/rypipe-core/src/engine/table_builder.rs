...
impl ColumnarSink for TableBuilder {
    ...
    fn put_field(&mut self, name: &str, value: Value<'_>) {
        if !self.reserve_field_memory(name.len(), &value) {
            return;
        }
    ...
    fn put_field_resolved(&mut self, resolved_name: &str, value: Value<'_>) {
        if !self.reserve_field_memory(resolved_name.len(), &value) {
            return;
        }
        if self.plan.filter.is_none() {
    ...
    fn put_field_at(&mut self, slot: u32, value: Value<'_>) {
        if !self.reserve_field_memory(0, &value) {
            return;
        }
    ...
    fn record_slot(&mut self, ordinal: u32, slot: u32, raw_name: &[u8]) {
        ...
        let growth = idx
            .saturating_add(1)
            .saturating_sub(self.ordinal_expect.capacity())
            .saturating_mul(std::mem::size_of::<Option<(u32, Vec<u8>)>>());
        if !self.reserve_memory(growth.saturating_add(raw_name.len())) {
            return;
        }
