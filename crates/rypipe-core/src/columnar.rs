...
mod scalar_parse;
pub(crate) use scalar_parse::{format_date32, format_decimal128, format_timestamp, parses_as};
pub use scalar_parse::{parse_date32, parse_decimal128, parse_timestamp};
...
impl ColumnBuilder {
    ...
    pub(crate) fn capacity_bytes(&self) -> usize {
        match self {
            ...
            ColumnBuilder::Timestamp(_, format, v) => {
                v.capacity_bytes() + format.as_ref().map_or(0, |s| s.len())
            }
            ...
            ColumnBuilder::Dictionary {
                ...
                codes.capacity_bytes()
                    + data.capacity()
                    + offsets.capacity() * 4
                    + index.capacity() * (std::mem::size_of::<(Box<str>, i32)>() + 1)
                    + index.keys().map(|s| s.len()).sum::<usize>()
            }
        }
    }
    ...
    pub(crate) fn to_arrow_array(&mut self) -> Result<ArrayRef> {
        ...
        Ok(match self {
            ...
            ColumnBuilder::Decimal128(scale, v) => {
                ...
                Arc::new(arr.with_precision_and_scale(38, (*scale).min(38) as i8)?)
            }
            ColumnBuilder::Dictionary {
                codes,
                data,
